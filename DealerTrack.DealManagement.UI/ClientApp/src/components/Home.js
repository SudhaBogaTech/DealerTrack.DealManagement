...
export class Home extends Component {
    ...
    handleUpload = () => {
        ...
        axios.post(`${this.state.url}/api/Deal/Create`, formData, config)
            .then((res) => {
                this.setState({
                    fileList: [],
                    uploading: false,
    ...
    getDeals = () => {
        ...
        axios.get(`${this.state.url}/api/Deal/GetAllDeals`, {
                ...
                <Row style={{ marginBottom: 16 }}>
                    ...
                    <Col span={12}>
                        <Card title="Most often sold vehicles">
                            {
                                this.state.vehicles.length > 0 ? this.state.vehicles.map((value, idx) => <p key={idx}>{value.name}</p>)
                                    : <div>No data yet</div>
                            }
                        </Card>
                    </Col>
